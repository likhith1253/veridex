...
class SettlementAccountingService:
    ...
    async def calculate_settlement_accounting(self) -> SettlementAccountingSummary:
        ...
        fees = Decimal(str(gw_fee or (gross * Decimal("0.02")).quantize(Decimal("0.01"))))
        taxes = Decimal(str(gw_tax or (fees * Decimal("0.18")).quantize(Decimal("0.01"))))
        ...
        return SettlementAccountingSummary(
            gross_gateway_volume=str(gross),
            total_deducted_fees=str(fees),
            total_deducted_taxes=str(taxes),
            total_refunded_amount=str(refunds),
            expected_net_settlement=str(expected_net),
            actual_bank_settled_credits=str(actual_bank),
            net_settlement_variance=str(variance),
            settlement_reconciliation_status=status,
            unsettled_delayed_exposure=str(unsettled),
        )
